...
!define STMNG_GITHUB_URL "https://github.com/crystalfp/STMng/releases/download/1.60.0/STMng-1.60.0-setup.exe"
...
; 7. STMng installation choice page (shown only if STMng is not installed)
Page custom PageSTMng PageSTMngLeave
...
Var PassiveMode
Var STMngChoice
Var STMngRadioGitHub
Var STMngRadioBundled
Var STMngRadioSkip
Var STMngInstallPath

Function PageSTMng
  ; Skip if STMng already installed
  IfFileExists "$PROGRAMFILES64\STMng\STMng.exe" 0 +2
    Abort
  IfFileExists "$PROGRAMFILES\STMng\STMng.exe" 0 +2
    Abort
  ; Skip in silent/passive mode
  IfSilent 0 +2
    Abort
  ${IfThen} $PassiveMode == 1 ${|} Abort ${|}

  !insertmacro MUI_HEADER_TEXT "STMng Visualizer" "Select installation method"

  nsDialogs::Create 1018
  Pop $0

  ${NSD_CreateLabel} 0 0 100% 30u "STMng visualizer is not installed.$\nChoose how you want to install it:"
  Pop $0

  ${NSD_CreateRadioButton} 10u 40u -10u 12u "Install from GitHub (version 1.60.0, requires internet)"
  Pop $STMngRadioGitHub

  ${NSD_CreateRadioButton} 10u 60u -10u 12u "Install bundled version (1.55.2, no internet required)"
  Pop $STMngRadioBundled

  ${NSD_CreateRadioButton} 10u 80u -10u 12u "Do not install STMng"
  Pop $STMngRadioSkip

  ${NSD_SetState} $STMngRadioGitHub ${BST_CHECKED}
  StrCpy $STMngChoice "1"

  nsDialogs::Show
FunctionEnd

Function PageSTMngLeave
  ${NSD_GetState} $STMngRadioGitHub $0
  ${If} $0 == ${BST_CHECKED}
    StrCpy $STMngChoice "1"
    Return
  ${EndIf}
  ${NSD_GetState} $STMngRadioBundled $0
  ${If} $0 == ${BST_CHECKED}
    StrCpy $STMngChoice "2"
    Return
  ${EndIf}
  StrCpy $STMngChoice "3"
FunctionEnd
...
Section STMngInstall
  StrCmp $STMngChoice "3" stmng_install_done 0
  StrCmp $STMngChoice "" stmng_install_done 0
  StrCmp $STMngChoice "1" stmng_install_github 0
  StrCmp $STMngChoice "2" stmng_install_bundled 0
  Goto stmng_install_done

  stmng_install_github:
    DetailPrint "Downloading STMng from GitHub..."
    Delete "$TEMP\STMng-setup.exe"
    NSISdl::download "${STMNG_GITHUB_URL}" "$TEMP\STMng-setup.exe"
    Pop $0
    ${If} $0 == "success"
      DetailPrint "Installing STMng..."
      HideWindow
      ExecWait '"$TEMP\STMng-setup.exe"'
      BringToFront
      Delete "$TEMP\STMng-setup.exe"
      Goto stmng_detect_path
    ${Else}
      Delete "$TEMP\STMng-setup.exe"
      MessageBox MB_ICONEXCLAMATION|MB_YESNO "Failed to download STMng from GitHub.$\n$\nWould you like to install the bundled version (1.55.2) instead?" IDYES stmng_install_bundled IDNO stmng_install_done
    ${EndIf}
    Goto stmng_install_done

  stmng_install_bundled:
    IfFileExists "$INSTDIR\STMng-1.55.2-setup.exe" 0 stmng_install_done
    DetailPrint "Installing bundled STMng..."
    HideWindow
    ExecWait '"$INSTDIR\STMng-1.55.2-setup.exe"'
    BringToFront

  stmng_detect_path:
    ; Try to find where STMng was installed
    ReadRegStr $STMngInstallPath HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\STMng" "InstallLocation"
    StrCmp $STMngInstallPath "" try_default_stmng_path 0
    StrCpy $STMngInstallPath "$STMngInstallPath\STMng.exe"
    IfFileExists "$STMngInstallPath" write_stmng_config 0
    try_default_stmng_path:
    StrCpy $STMngInstallPath "$PROGRAMFILES64\STMng\STMng.exe"
    IfFileExists "$STMngInstallPath" write_stmng_config 0
    StrCpy $STMngInstallPath "$PROGRAMFILES\STMng\STMng.exe"
    IfFileExists "$STMngInstallPath" write_stmng_config 0
    Goto stmng_install_done

  write_stmng_config:
    ; Write PowerShell script to update USPEX Runner config.json with stmng_path
    FileOpen $0 "$TEMP\uspex_stmng_update.ps1" w
    FileWrite $0 '$$confDir = "$APPDATA\USPEX Runner"$\r$\n'
    FileWrite $0 '$$confFile = Join-Path $$confDir "config.json"$\r$\n'
    FileWrite $0 '$$stmngPath = "$STMngInstallPath"$\r$\n'
    FileWrite $0 'if (Test-Path $$confFile) { $$c = Get-Content $$confFile -Raw | ConvertFrom-Json } else { New-Item -ItemType Directory -Force -Path $$confDir | Out-Null; $$c = New-Object PSObject }$\r$\n'
    FileWrite $0 '$$c | Add-Member -NotePropertyName stmng_path -NotePropertyValue $$stmngPath -Force$\r$\n'
    FileWrite $0 '$$enc = New-Object System.Text.UTF8Encoding $$false$\r$\n'
    FileWrite $0 '[System.IO.File]::WriteAllText($$confFile, ($$c | ConvertTo-Json -Depth 10), $$enc)$\r$\n'
    FileClose $0
    ExecWait 'powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "$TEMP\uspex_stmng_update.ps1"'
    Delete "$TEMP\uspex_stmng_update.ps1"
